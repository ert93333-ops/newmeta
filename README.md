...
For local development without a Supabase project, keep `HERMES_AUTH_MODE=mock`. Production refuses mock auth. `/api/me` can bootstrap the authenticated user's tenant memberships from a Supabase bearer token; tenant-scoped routes require the bearer token plus `x-tenant-id`. `GET /api/tenants/:id` returns only the caller's membership-scoped tenant metadata and denies non-member tenants. `POST /api/drafts/preflight` remains analysis-only but still requires tenant auth. `POST /api/placement/validate` now persists tenant-scoped `placement_validation_reports` rows and writes an audit log instead of returning an ephemeral validation result only. `POST /api/bottleneck/jobs` now persists `bottleneck_analysis_jobs`, `bottleneck_stage_scores`, and derived `bottleneck_hypotheses` rows under the same job id that the generic `/api/jobs/:id` surface returns. `POST /api/performance-fusion/reports` now persists tenant-scoped fusion reports instead of returning report JSON only. `PATCH /api/settings/*` now persists tenant-scoped settings through `integration_settings` and requires `marketer` or above. `POST /api/drafts/create-paused` reruns preflight on the server, creates a pending `meta_create_ad_paused` approval when needed, and only persists an `ad_drafts` record after an approved same-tenant request is supplied. `POST /api/cost/estimate` reads provider pricing and caps from the tenant's stored `integration_settings` row keyed by `settings.providerName` and fails closed when that row is missing. `GET /api/meta/ad-accounts` and `POST /api/meta/sync/insights` now resolve their Meta adapter server-side: they use the latest tenant-scoped encrypted `meta_connections` record when present, and only fall back to `MockMetaAdapter` outside production when no live connection exists.
