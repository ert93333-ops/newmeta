...
`POST /api/creative-analysis/jobs` is an authenticated tenant-scoped persistence route. It now requires `asset.id` from an already persisted creative asset. On success, it mirrors the same id into the generic `/api/jobs/:id` surface and the dedicated `creative_analysis_jobs` row, stores extracted feature blobs in `creative_features`, stores numeric score rows in `creative_component_scores`, stores `video_segments` for video assets, and writes an audit log. Requests without a persisted asset id fail closed with `CREATIVE_ASSET_ID_REQUIRED`.
