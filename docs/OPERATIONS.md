...
## Autopilot Guideline Application

Hermes applies the Meta Ads autopilot guideline in the current release as a proposal-only control plane:

- Default autonomy is `RECOMMENDATION`, matching the guideline's early-stage `PROPOSE_ONLY` rollout.
- Autopilot recommendation output is shaped as a decision proposal for a single-writer `action_orchestrator`; no UI panel, API route, worker, or agent may bypass that owner for Meta write actions.
- Data quality gates hold proposals when ad-level insights are missing or stale, when ad-to-creative metadata joins are incomplete, or when budget mutation fields appear.
- Creative experiments must change one variable at a time and carry fixed variables, primary metric, guardrail metrics, minimum sample, and winner-stop conditions.
- Creative generation may accept a reference product image URL and product homepage URL. `POST /api/product-references/extract` prepares these inputs server-side before paid generation approval: it fetches only homepage HTML with timeout/content-type/byte caps, extracts conservative title/meta/OG/JSON-LD Product facts, returns a product-only instruction, and writes an audit breadcrumb without storing raw HTML. The approval stores this as `generationContext`; after approval, the render queue passes it to the worker, and successful provider output is registered as `creative_assets` candidates for the existing PAUSED draft route. The approved generation job must isolate the product as the hero subject, ignore unrelated page chrome/background/reviews/navigation, preserve only verifiable product facts, and generate multiple controlled variants through the cost-guarded `units` count.
- Publish, paid generation, destructive work, and any risky Meta status change remain approval-gated. Budget execution remains unavailable; budget guidance is text-only.
- Full `LIMITED_AUTO`, bandit, Bayesian optimization, and shadow RL behavior stay future-phase work until the guideline's shadow-mode, rollback, rejection-rate, reconciliation, and spend-anomaly criteria are met.
