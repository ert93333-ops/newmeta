...
- `tenant_data_deletion` is also no longer executable through the generic `POST /api/approvals/:id/execute` route. Hermes must not report a successful destructive delete until a dedicated deletion executor exists.
