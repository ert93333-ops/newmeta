...
- Paid image/video generation queueing and worker execution are local-mock only until a real provider is wired; production API and worker paths fail closed with `PAID_GENERATION_WORKER_NOT_CONFIGURED` instead of consuming approvals or recording deterministic mock output as paid production output.
