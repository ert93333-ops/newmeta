...
`GET /api/meta/signal-diagnostics?adAccountId=...` uses the same server-side adapter resolution. In live mode it reads Pixel readiness from Meta Graph `/{adAccountId}/adspixels` with a server-only bearer token. CAPI and GA4 readiness are overlaid from tenant `integration_settings` provider `signal-diagnostics`; it only uses non-secret readiness fields such as dataset/property/measurement ids and server-access configured booleans. Live diagnostics require an explicit ad account id; local mock mode may default to `act_mock_001`.
